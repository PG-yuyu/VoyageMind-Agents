...
class IntentService:
    ...
    def detect(self, message: str) -> IntentResult:
        ...
        if "预算" in text and any(
            word in text for word in ["改", "调整", "降低", "提高", "降到", "改成"]
        ):
            return IntentResult(
                intent="modify_trip",
                confidence=0.9,
        ...
        for keyword, value in self.modification_words.items():
            if (
                keyword in text
                and has_change_signal
                and not (has_create_signal and "改" not in text)
            ):
                sub_intent = value
                return IntentResult(
                    intent="modify_trip",
        ...
        if any(word in text for word in self.qa_words) and not any(
            word in text for word in ["规划", "生成"]
        ):
