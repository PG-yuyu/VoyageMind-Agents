...
现有 `langchain-chat-main` 和 `LangChain_RAG-main` 不在这里重写：

- `langchain-chat-main`：作为 Chatbot / LLM 调用底座复用。
...
前端会通过 Vite 代理访问 `/api/v1`，对应后端 `http://127.0.0.1:8001/api/v1`。
...
`ChatbotService` 位于 `backend/services/chatbot_service.py`，它复用现有
`langchain-chat-main` 的 `ChatEngine`。如果本机模型依赖或 Key 未配置，会自动规则兜底，
